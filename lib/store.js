...
const db = getDb();
const otpEncryptionKey = crypto.createHash('sha256').update(process.env.OTP_ENCRYPTION_KEY || process.env.ORDER_ENCRYPTION_KEY || '').digest();
...
const productSelect = db.prepare(`
  SELECT id, title, price, tag, rating, stock, category, game_id as gameId, image_url as imageUrl, description,
         platform, launcher, requirements, delivery_time as deliveryTime, warranty_days as warrantyDays,
         original_price as originalPrice, service_status as serviceStatus, fulfillment_method as fulfillmentMethod,
         important_notes as importantNotes, supported_regions as supportedRegions, availability
  FROM products
  ORDER BY id ASC
`);
...
const createProductStatement = db.prepare(`
  INSERT INTO products (title, price, tag, rating, stock, category, game_id, image_url, description, platform, launcher, requirements, delivery_time, warranty_days, original_price, service_status, fulfillment_method, important_notes, supported_regions, availability)
  VALUES (@title, @price, @tag, @rating, @stock, @category, @gameId, @imageUrl, @description, @platform, @launcher, @requirements, @deliveryTime, @warrantyDays, @originalPrice, @serviceStatus, @fulfillmentMethod, @importantNotes, @supportedRegions, @availability)
`);
...
const productByIdSelect = db.prepare(`
  SELECT id, title, price, tag, rating, stock, category, game_id as gameId, image_url as imageUrl, description,
         platform, launcher, requirements, delivery_time as deliveryTime, warranty_days as warrantyDays,
         original_price as originalPrice, service_status as serviceStatus, fulfillment_method as fulfillmentMethod,
         important_notes as importantNotes, supported_regions as supportedRegions, availability
...
const ordersByUserSelect = db.prepare(`
  SELECT id, name, email, game, launcher, launcher_id as launcherId, note, amount_paise as amountPaise,
         ...
         platform_type, service_type, delivery_eta, game_id as gameId, delivered_at as deliveredAt
  FROM orders 
  WHERE email = @email
  ORDER BY created_at DESC
`);
...
export async function createProduct(payload) {
  const productEntry = {
    ...
    platform: String(payload.platform || 'PC').trim(),
    launcher: String(payload.launcher || 'Steam').trim(),
    requirements: String(payload.requirements || '').trim(),
    deliveryTime: String(payload.deliveryTime || payload.delivery_time || '2-4 hrs').trim(),
    warrantyDays: Number.parseInt(payload.warrantyDays ?? payload.warranty_days ?? 30, 10) || 30,
    originalPrice: String(payload.originalPrice || payload.original_price || '').trim(),
    serviceStatus: String(payload.serviceStatus || payload.service_status || 'active').trim(),
    fulfillmentMethod: String(payload.fulfillmentMethod || payload.fulfillment_method || 'account_login').trim(),
    importantNotes: String(payload.importantNotes || payload.important_notes || '').trim(),
    supportedRegions: String(payload.supportedRegions || payload.supported_regions || '').trim(),
    availability: String(payload.availability || 'available').trim(),
  };
...
export async function updateOrderFulfillmentStatus(orderId, status, agentId = null) {
  ...
  if (status === 'delivered') {
    updateQuery += `, sensitive_data_purged = 1, delivered_at = @deliveredAt`;
    params.deliveredAt = new Date().toISOString();
    if (params.agentId) {
...
export async function sendDiscordWebhook(webhookUrl, orderData) {
  ...
  const message = {
    content: `ðŸ”” **New GTA V Service Order**

ðŸ†” **Order ID:** ${orderData.id}
ðŸ›’ **Service:** ${orderData.serviceType || 'Account Recovery'}
ðŸ–¥ï¸ **Platform:** ${orderData.platformType || 'Not specified'}
ðŸ‘¤ **Customer:** ${orderData.discordUsername || orderData.name} (${orderData.email})
ðŸ’° **Total:** â‚¹${(orderData.amountPaise / 100).toLocaleString('en-IN')}
ðŸ“¦ **Status:** ${orderData.status}
â° **Time:** ${new Date(orderData.createdAt).toLocaleString('en-IN')}`
  };
...
export async function createOrder(payload) {
  ...
  const orderEntry = {
    ...
    status: 'pending',
    createdAt: new Date().toISOString(),
    platformType: String(payload.platformType || payload.platform_type || '').trim(),
    discordUsername: String(payload.discordUsername || payload.discord_username || '').trim(),
    whatsappPhone: String(payload.whatsappPhone || payload.whatsapp_phone || '').trim(),
    deliveryEta: String(payload.deliveryETA || payload.delivery_eta || '').trim(),
    gameId: String(payload.gameId || payload.game_id || 'gta5').trim(),
  };
...
const getUserByEmailWithPasswordStatement = db.prepare('SELECT id, name, email, password_hash FROM users WHERE email = @email');
const createUserWithPasswordStatement = db.prepare(`
  INSERT INTO users (name, email, password_hash)
  VALUES (@name, @email, @passwordHash)
`);

export async function getUserByEmailWithPassword(email) {
  return getUserByEmailWithPasswordStatement.get({ email }) || null;
}

export async function createUserWithPassword(name, email, passwordHash) {
  const result = createUserWithPasswordStatement.run({ name, email, passwordHash });
  return {
    id: Number(result.lastInsertRowid),
    name,
    email,
...
const insertPaymentEvent = db.prepare(`
  INSERT OR IGNORE INTO payment_events (event_id, event_type, razorpay_order_id, razorpay_payment_id, payload, processed)
  VALUES (@eventId, @eventType, @razorpayOrderId, @razorpayPaymentId, @payload, @processed)
`);

const getPaymentEventByEventId = db.prepare('SELECT * FROM payment_events WHERE event_id = @eventId');

const markPaymentEventProcessed = db.prepare('UPDATE payment_events SET processed = 1 WHERE id = @id');

const getOrderByRazorpayOrderId = db.prepare('SELECT id, email, status, amount_paise as amountPaise, razorpay_payment_id as razorpayPaymentId FROM orders WHERE razorpay_order_id = @razorpayOrderId');

export function recordPaymentEvent(eventId, eventType, razorpayOrderId, razorpayPaymentId, payload) {
  const existing = getPaymentEventByEventId.get({ eventId });
  if (existing) {
    return { duplicate: true, id: existing.id };
  }

  const result = insertPaymentEvent.run({
    eventId,
    eventType,
  ...
  return { duplicate: false, id: Number(result.lastInsertRowid) };
}

export function markPaymentEventDone(id) {
  markPaymentEventProcessed.run({ id });
}

export function findOrderForWebhook(razorpayOrderId) {
  return getOrderByRazorpayOrderId.get({ razorpayOrderId }) || null;
}

export function markOrderPaidIfPending(orderId, razorpayPaymentId) {
  const order = db.prepare('SELECT id, status FROM orders WHERE id = @id').get({ id: orderId });
  if (!order) return { updated: false };
  if (order.status === 'paid' || order.status === 'delivered') return { updated: false, alreadyPaid: true };
  markOrderPaidStatement.run({ id: orderId, razorpayPaymentId });
  return { updated: true };
}
